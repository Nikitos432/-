...
def delete_car(file):
    while True:
        ...
        if del_znach == '9':
            i = 0
            while True:
                del_manufacturer = input('Введите активность дверей(да|нет) удаляемой машины: ').lower()
                del_numbers_lst = []
                for key_1, value_1 in enumerate(lines):
    ...
    while True:
        ...
        if str_count.isdigit():
            if str_count == '1':  # Поиск по производителю
                ...
                while True:
                    del_manufacturer = input('Введите активность дверей (да/нет): ').lower().strip()
                    _lst = []  # Очищаем список перед каждым новым поиском
                    for line in lines:
                        if len(line) > 8 and line[8].strip().lower() == del_manufacturer:
                            _lst.append(line)
                    if not _lst:
                        print('Нет машин с такой активностью дверей.')
                        a = input('Введите другую активность дверей либо для выхода напишите отмена: ').lower()
                        if a == 'отмена':
                            break
                    else:
                        print(tabulate(_lst, headers=table_title, tablefmt="github"))
